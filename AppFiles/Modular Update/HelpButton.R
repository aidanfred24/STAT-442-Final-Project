      ...
      observeEvent(input$HelpButton, {
        ...
        if (currentTab() == "statewise") {
          ...
          text <- "<div style = 'text-align: center;'> The Facility Profile tab attempts to give a broad overview
                   ...
                   <div><br><b>Useful Info for Highlighted Areas:</b><br>
                      <ul><li><b>#1: </b>Select a state/territory of interest, which must be selected prior to
                      selecting a facility</li>
                      <li><b>#2: </b>Select a facility of interest. Some facilities report for multiple locations,
                      which can be seen on the timeline if that is the case</li>
                      <li><b>#3: </b>Select a metric for the timeline on the far right. Metrics include totals and
                      proportions/percentages of release, recycling, waste, etc.</li>
                      <li><b>#4: </b>Perform address search for selected facility. After selecting one of the 
                      facility's locations (left), press the search button (right) to open the Address Search tab
                      and fill the address box with the location's address</li>
                      <li><b>#5: </b>Chemical Checkist. Displays which chemical types the selected facility 
                      releases. Chemical Descriptions: 
                        <ul><li><b>CAA Hazardous: </b>Declared hazardous by the Clean Air Act</li>
                        <li><b>Carcinogen: </b>Chemicals linked to cancer development</li>
                        <li><b>PBT Chemical: </b>Persistent, bioaccumulative, and toxic</li>
          ...
          text <- "<div style = 'text-align: center;'>The Address Search tab allows for users to enter in any US address in order to gain
                  ...
                  <br><br>Each point on the map represents an individual facility, while the large black marker
                  represents either the address or facility that a user searches. Each point can be clicked to
                  reveal more information about the facility it represents.</div>
                  <div><br><b>Tab Elements: </b>
                      <ul><li><b>#1: </b>Address input section. A good format to follow for entering an address
                      is '[# and Street], [City], [State]', but it will also work if only a city and state is entered</li>
                      <li><b>#2: </b> Adjust radius of search. Radius can be set from 0 to 100 US survey miles 
                      from the given address</li>
                      <li><b>#3: </b>Legend for generated plot. Measures the Average Yearly Chemical release of displayed 
                      facilities</li></ul></div>
                      <div><b>Warning: </b>It is <b>IMPORTANT</b> not to confuse what this scale
                      applies to. The legend on the plot corresponds to the color of the points that represent each 
                      facility. The color of the heatmap element (the blended colors) should be used only as a reference
